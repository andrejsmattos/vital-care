...
export class ProntuarioService {
  ...
  getProntuarioDePaciente(id: string): Observable<Exame[]> {
    ...
    return this.http.get<Exame[]>(`${this.urlPath}/${id}/prontuarios`, {
      headers,
    });
  }
  ...
  getExamesPaciente(id: string): Observable<Exame[]> {
    const headers = this.authService.getAuthHeaders();
    return this.http
      .get<{ exames: Exame[] }>(`${this.urlPath}/${id}/prontuarios`, {
        headers,
      })
      .pipe(map((response) => response.exames));
  }

  getConsultasPaciente(id: string): Observable<Consulta[]> {
    const headers = this.authService.getAuthHeaders();
    return this.http
      .get<{ consultas: Consulta[] }>(`${this.urlPath}/${id}/prontuarios`, {
        headers,
      })
      .pipe(map((response) => response.consultas));
  }
