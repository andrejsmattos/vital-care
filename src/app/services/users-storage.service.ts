...
export class UserStorageService {
  ...
  urlPath: string = `${environment.apiUrl}/usuarios`;
  ...
  addUser(user: any): Observable<User> {
    ...
    return this.http.post<User>(`${this.urlPath}/pre-registro`, user, {
      headers,
    });
  }
  ...
  getUsersByEmailOrById(buscaInput: string) {
    ...
    if (this.isNumeric(buscaInput)) {
      return this.http.get<User>(`${this.urlPath}?id=${buscaInput}`, {
        headers,
      });
    } else {
